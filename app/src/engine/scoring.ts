// AUDIT-C scoring — TypeScript port of tools/scoring.mjs (pure functions,
// no I/O, no state). Thresholds are CONFIG VALUES
// (config/onboarding.v1.json -> scoring.audit_c.bands), passed in by the
// caller — never hard-coded here. The shipped bands are PROVISIONAL pending
// clinical review. Mechanism only — safety copy and routing are authored
// design-side and are not this module's concern.
import type { Answers, FlowConfig, InstrumentBands } from "./types";
 ...
 * Returns { score, band } (caller supplies the gender answer when the flow captured one).
 */
export function scoreInstrument(
  ...
  for (const id of inst.sum_of) {
    ...
    if (!opt || opt.score === undefined) {
      throw new Error(`answer ${JSON.stringify(a.value)} on ${id} has no score`);
    }
    score += opt.score;
  }
